...
public class VirusScanner {
	...
	public static final ScheduledExecutorService schedule = Executors.newScheduledThreadPool(5);
	...
	public void scanDesk() {
		...
		final Runnable scanner = new Runnable() {
			...
			@Override public void run() {
				...
				try {
					...
					statusString.setText("scan ->" +
							scanNumber + "start at->" + df.format(cal.getTime()));
		...
		//重点
		final ScheduledFuture<?> scheduleManager = schedule.scheduleAtFixedRate(scanner, 1, 15, TimeUnit.SECONDS);
